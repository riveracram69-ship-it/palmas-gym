...
<!-- ── Log Workout Modal Drawer ── -->
<div class="notif-overlay" id="workout-overlay" onclick="closeWorkoutModal()"></div>
<div class="notif-drawer" id="workout-drawer" style="width:min(420px, 100vw);">
    <div class="notif-drawer-header">
        <h3><i class="fas fa-dumbbell" style="color:var(--palmas-primary);"></i> Log Workout Session</h3>
        <button class="notif-drawer-close" onclick="closeWorkoutModal()"><i class="fas fa-xmark"></i></button>
    </div>
    <div style="padding:1.25rem; flex:1; overflow-y:auto;">
        <form id="workout-log-form" onsubmit="submitWorkoutLog(event)">
            <!-- Workout Type Selection -->
            <label style="display:block; font-size:0.82rem; font-weight:700; color:var(--text-primary); margin-bottom:0.5rem;">
                Workout Type
            </label>
            <div style="display:grid; grid-template-columns: repeat(3, 1fr); gap:0.5rem; margin-bottom:1.25rem;" id="workout-type-selector">
                <button type="button" class="workout-type-btn active" data-type="Strength" onclick="selectWorkoutType('Strength')">
                    <i class="fas fa-dumbbell"></i>
                    <span>Strength</span>
                </button>
                <button type="button" class="workout-type-btn" data-type="Cardio" onclick="selectWorkoutType('Cardio')">
                    <i class="fas fa-person-running"></i>
                    <span>Cardio</span>
                </button>
                <button type="button" class="workout-type-btn" data-type="HIIT" onclick="selectWorkoutType('HIIT')">
                    <i class="fas fa-bolt"></i>
                    <span>HIIT</span>
                </button>
                <button type="button" class="workout-type-btn" data-type="Functional" onclick="selectWorkoutType('Functional')">
                    <i class="fas fa-arrows-spin"></i>
                    <span>Functional</span>
                </button>
                <button type="button" class="workout-type-btn" data-type="Yoga" onclick="selectWorkoutType('Yoga')">
                    <i class="fas fa-spa"></i>
                    <span>Yoga</span>
                </button>
                <button type="button" class="workout-type-btn" data-type="Boxing" onclick="selectWorkoutType('Boxing')">
                    <i class="fas fa-hand-back-fist"></i>
                    <span>Boxing</span>
                </button>
            </div>
            <input type="hidden" id="workout-type-val" value="Strength">
            ...
            <div style="display:grid; grid-template-columns: 1fr 1fr; gap:0.75rem; margin-bottom:1.25rem;">
                <div>
                    ...
                    <input type="number" id="workout-duration-val" min="5" max="360" value="45" required
                           style="width:100%; padding:0.65rem 0.85rem; border:1.5px solid var(--border); border-radius:10px; font-size:0.95rem; font-weight:600; outline:none; box-sizing:border-box;">
                </div>
                <div>
                    <label style="display:block; font-size:0.82rem; font-weight:700; color:var(--text-primary); margin-bottom:0.4rem;">
                        Calories (approx)
                    </label>
                    <input type="number" id="workout-calories-val" min="0" max="3000" placeholder="Optional"
                           style="width:100%; padding:0.65rem 0.85rem; border:1.5px solid var(--border); border-radius:10px; font-size:0.95rem; font-weight:600; outline:none; box-sizing:border-box;">
                </div>
            </div>
            ...
            <div style="margin-bottom:1.5rem;">
                <label style="display:block; font-size:0.82rem; font-weight:700; color:var(--text-primary); margin-bottom:0.4rem;">
                    Session Notes (Exercises / PRs)
                </label>
                <textarea id="workout-notes-val" rows="3" placeholder="e.g. Bench press 4x8 @ 80kg, Squats 4x10..."
                          style="width:100%; padding:0.65rem 0.85rem; border:1.5px solid var(--border); border-radius:10px; font-size:0.88rem; outline:none; resize:none; font-family:inherit; box-sizing:border-box;"></textarea>
            </div>

            <!-- Submit Button -->
            <button type="submit" id="save-workout-btn" class="renew-btn" style="width:100%; margin:0;">
                <i class="fas fa-check-circle"></i> Save Workout Log
            </button>
        </form>
    </div>
</div>
...
.workout-type-btn.active {
    ...
    color: var(--palmas-primary-deep);
}
.workout-type-btn.active i {
...
function openWorkoutModal() {
    ...
    document.getElementById('workout-drawer').classList.add('active');
}

function closeWorkoutModal() {
    document.getElementById('workout-overlay').classList.remove('active');
    document.getElementById('workout-drawer').classList.remove('active');
}

function selectWorkoutType(type) {
    document.getElementById('workout-type-val').value = type;
...
async function loadWorkoutStats() {
    try {
        const res = await fetch('../api/member_workouts.php');
        const json = await res.json();
        if (json.success && json.data) {
            const d = json.data;
            document.getElementById('dash-workout-streak').textContent = d.streak || 0;
            document.getElementById('dash-workout-week-count').textContent = d.this_week.completed || 0;
            document.getElementById('dash-workout-week-target').textContent = d.this_week.target || 4;
            
            const pct = d.this_week.percentage || 0;
            document.getElementById('dash-workout-progress-pct').textContent = pct + '%';
            document.getElementById('dash-workout-progress-bar').style.width = pct + '%';

            // Populate recent activity mini list
            ...
            if (d.workouts && d.workouts.length > 0) {
                const iconMap = {
                    ...
                    'HIIT': 'fa-bolt',
                    'Functional': 'fa-arrows-spin',
                    'Yoga': 'fa-spa',
                    'Boxing': 'fa-hand-back-fist',
                    'General': 'fa-heart-pulse'
                };
                let html = '<div style="display:flex; flex-direction:column; gap:0.45rem;">';
                d.workouts.slice(0, 3).forEach(w => {
                    const icon = iconMap[w.workout_type] || 'fa-dumbbell';
                    const dateStr = new Date(w.logged_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
                    html += `
                        <div style="display:flex; align-items:center; justify-content:space-between; padding:0.45rem 0.6rem; background:#ffffff; border:1px solid #edf4ee; border-radius:8px; font-size:0.8rem;">
                            <div style="display:flex; align-items:center; gap:0.55rem;">
                                <div style="width:28px; height:28px; border-radius:50%; background:#f0fdf4; color:var(--palmas-primary); display:flex; align-items:center; justify-content:center; font-size:0.75rem;">
                                    <i class="fas ${icon}"></i>
                                </div>
                                <div>
                                    <span style="font-weight:700; color:var(--text-primary);">${w.workout_type}</span>
                                    <span style="font-size:0.72rem; color:#9ca3af; margin-left:0.35rem;">${w.duration_minutes} mins</span>
                                </div>
                            </div>
                            <span style="font-size:0.72rem; color:#6b7280; font-weight:600;">${dateStr}</span>
                        </div>
                    `;
                });
                html += '</div>';
                ...
                container.innerHTML = `
                    <div style="font-size:0.82rem; color:#9ca3af; text-align:center; padding:0.5rem 0;">
                        No workouts logged this week. Tap "+ Log Workout" to start!
                    </div>
                `;
            }
        }
...
async function submitWorkoutLog(e) {
    e.preventDefault();
    ...
    const payload = {
        action: 'log_workout',
        workout_type: document.getElementById('workout-type-val').value,
        duration_minutes: document.getElementById('workout-duration-val').value,
        calories_burned: document.getElementById('workout-calories-val').value || 0,
        notes: document.getElementById('workout-notes-val').value.trim()
    };
    ...
    try {
        ...
        if (data.success) {
            closeWorkoutModal();
            showToast(data.message || 'Workout logged!');
            document.getElementById('workout-notes-val').value = '';
            loadWorkoutStats();
        } else {
...
// Load workouts on DOM ready
document.addEventListener('DOMContentLoaded', loadWorkoutStats);
</script>
